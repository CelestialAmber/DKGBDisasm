...
static void usage(void) {
	fprintf(stderr, "Usage: gfx [--trim-whitespace] [--remove-whitespace] [--interleave] [--remove-duplicates [--keep-whitespace]] [--remove-xflip] [--remove-yflip] [--png filename] [-d depth] [-w whitespace] [-h] [-o outfile] infile\n");
}
...
void get_args(int argc, char *argv[]) {
	struct option long_options[] = {
		...
		{"add-whitespace", required_argument, 0, 'w'},
		{"help", no_argument, 0, 'h'},
		{0}
	};
	for (int opt = 0; opt != -1;) {
		switch (opt = getopt_long(argc, argv, "ho:d:p:w:", long_options)) {
		case 'h':
			Options.help = true;
			break;
		case 'o':
		...
		case 'p':
			...
			break;
		case 'w':
			Options.whitespace_amount = strtoul(optarg, NULL, 0);
			break;
		case 0:
		case -1:
			break;
...
void add_whitespace(struct Graphic *graphic, int originalSize) {
	int totalWhitespaceBytes = Options.depth * 8 * Options.whitespace_amount;
	int newSize = graphic->size + totalWhitespaceBytes;
	if(newSize > originalSize){
		graphic->data = realloc(graphic->data,newSize);
	}
	for(int i = graphic->size ; i < newSize; i++){
		graphic->data[i] = 0;
	}
	graphic->size = newSize;
}
...
int main(int argc, char *argv[]) {
	...
	if(Options.whitespace_amount != 0){
		int originalSize = graphic.size;
		trim_whitespace(&graphic);
		add_whitespace(&graphic,originalSize);
	}
